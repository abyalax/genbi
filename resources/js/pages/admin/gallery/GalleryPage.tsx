...
import { Gallery } from "@/types/news"
                ...
                <>
                    <div className="grid grid-cols-4 grid-rows-3 justify-center items-center gap-x-8 gap-y-4 p-6 w-full">
                        ...
                        {gallery.map((item, index) => (
                            <div className="col-span-1 w-72 h-56 bg-white rounded-md p-2 border-2 border-slate-200" key={index} onClick={() => router.visit(`/news/${item.slug}`)}>
                                <img src={item.image} alt="kegiatan" width={500} height={300} className="w-64 h-44 mx-auto object-cover object-center rounded-md" />
                                <p className="text-center my-2 text-sm font-semibold">{item.title.length > 30 ? item.title.substring(0, 30) + "..." : item.title}</p>
                            </div>
                        ))}
