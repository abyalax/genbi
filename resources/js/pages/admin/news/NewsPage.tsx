...
export default function NewsPage({ news, auth }: PageProps<{ news: News[] }>) {
    ...
    const handleCreateNews = () => {
        setWindow({
            windowElement: {
                ...
                children: <CreateNews id={auth.user.id} name={auth.user.name}/>
            }
        })
    }
    ...
    return (
        <AdminLayout>
            <div className="flex gap-4 w-full justify-between my-4 pr-8">
                ...
                <button onClick={handleCreateNews} className="flex h-fit items-center gap-2 py-2 px-4 bg-toska text-white hover:bg-toska-dark rounded-md">
