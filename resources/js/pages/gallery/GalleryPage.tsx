      ...
      <main className="w-full bg-[#edf0f7] min-h-screen">
        ...
        <section>
          <div className="flex flex-wrap md:gap-14 sm:gap-12 xxs:gap-8 gap-8 justify-center items-start ml:px-20 md:px-14 sm:px-12 xxs:px-8 px-2 pt-6">
            {gallery?.map((e, i) => (
              <div key={i} className="h-52 w-80" onClick={() => router.visit(`/news/${e.slug}`)}>
                {e.image ? (
                  <img src={e.image} alt={e.title} width={500} height={500} className="w-full h-full object-center object-cover rounded" />
                ): (
                <div className="h-52 w-80 p-4 rounded-lg bg-slate-400"></div>
                )}
                <p className="text-center text-gray-700 mb-4 mt-2">{e.title}</p>
              </div>
            ))}
          </div>
        </section>
