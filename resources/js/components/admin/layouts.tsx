...
const AdminLayout = ({ children }: { children: ReactNode }) => {
    ...
    const getToken = () => {
        const res = fetchAxios.get(`/tokens/create`)
        res.then((res) => {
            console.log(res);
            const token = res.data.token
            fetchAxios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            localStorage.setItem("token", token)
        })
        res.catch((res) => {
            console.log(res);
            delete fetchAxios.defaults.headers.common['Authorization'];
        ...
        <section className="flex w-full min-h-screen text-gray-700">
            <aside className="w-fit flex flex-col justify-between bg-white p-3 h-screen fixed">
                <div className="pt-8 px-4">
                    ...
                    <p className="font-semibold text-gray-500 text-nowrap">Management System</p>
                ...
                <div className="border-t border-slate-200 flex justify-center items-center p-4 w-full">
                    <button onClick={() => router.post(route('logout'))} className="flex items-center justify-center gap-4 py-2 w-full text-lg bg-toska-light text-white hover:bg-red-500 rounded">
            ...
            <main className="min-w-full bg-[#edf0f7] py-6 pr-6 pl-72">
                {children}
            </main>
