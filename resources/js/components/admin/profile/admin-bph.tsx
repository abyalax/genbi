                ...
                <div className="border border-slate-300 rounded-lg p-4 m-4 flex gap-9">
                    <div className="flex flex-col items-center w-full">
                        <div className="w-full p-2">
                            ...
                            <ul className="mb-4 hover:bg-slate-300 cursor-pointer rounded-lg" onClick={() => router.visit(`/admin/profile/${ketua.id}`)}>
                                <li className="my-2 flex justify-between border-b border-slate-300 p-2">
                                    <div className="flex gap-4">
                                        {ketua.image ? (
                                            <img src={ketua.image as string} alt="" width={200} height={200} className="object-cover object-center w-20 h-20 aspect-square rounded-full" />
                                        ) : (
                            ...
                            <ul>
                                {member.map((e, index) => (
                                    <li key={index} className="my-2 flex justify-between border-b border-slate-300 p-2 cursor-pointer hover:bg-slate-300 rounded-lg" onClick={() => router.visit(`/admin/profile/${e.id}`)}>
                                        <div className="flex gap-4" >
                                            {ketua.image ? (
                                                <img src={e.image as string} alt="" width={200} height={200} className="object-cover object-center w-20 h-20 aspect-square rounded-full" />
                                            ) : (
                                                <DefaultProfile size={"sm"} />
                                            )}
                                            <div className="">
